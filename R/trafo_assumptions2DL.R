###NOTE THIS DOES NOT WORK IN R version 4.2.1 and after because of {rrdr}check_assumptions
#This modification has an extra input plotit=TRUE or FALSE, default FALSE.
#Make sure to also do the environment command at the end during your session so
#it can call other functions from the trafo package as the original command called
#assumptions() does. Reason: graphinc the nested scatter plots for every non-helpful
#transformation takes a very long time. Once you see a transformation that improves
#homoschedasticity, skew and kurtosis, then you can set plotit=True. D Levine 12/8/2022.
...
assumptions2 <- function(object, method = "ml", std = FALSE, plotit=FALSE,...){
  ...
  pairs(formula(object$terms), data = object$model, main = "Untransformed model", 
        lower.panel = panel.smooth, upper.panel = panel.cor)} else {print(summary(object))}
  ...
  for (transform in names(trafos)) {
    ...
    } else {print(paste(transform,"r2=", max(0,1-(sum((resid[[transform]])^2)/sum((trafo_mod[[transform]]$fitted.values-mean(trafo_mod[[transform]]$fitted.values, na.rm=TRUE))^2)))))
  }
  }
  
}
##To make it call other trafo functions, 

environment(assumptions2)<-asNamespace("trafo")

##not needed here but if in future you need it, 
##to make this function be callable inside your session by other trafo functions

#assignInNamespace("assumptions",assumptions2,ns="trafo")
